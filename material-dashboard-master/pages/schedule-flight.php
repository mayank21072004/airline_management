...
<?php
  ...
  if (isset($_POST['submit'])) 
  {
    ...
    $status=False;
    // echo "<script>alert('Entered in if $adatetimeFormatted');</script>";
    // Check if any rows are fetched
    if (mysqli_num_rows($result) > 0) 
    {
        // Loop through each fetched row
        // echo "<script>alert('enter in checking');</script>";
        while ($row = mysqli_fetch_assoc($result)) {
          // echo "<script>alert('Entered in while');</script>";
            // Check if the new flight schedule overlaps with any existing schedule
            if (($adatetimeFormatted >= $row['adatetime'] && $adatetimeFormatted <= $row['ddatetime']) || ($ddatetimeFormatted >= $row['adatetime'] && $ddatetimeFormatted <= $row['ddatetime']) ||($adatetimeFormatted <= $row['adatetime'] && $ddatetimeFormatted >= $row['ddatetime'])) {                
                echo "<script>alert('This Flight is Already Scheduled between {$row['adatetime']} and {$row['ddatetime']}');</script>";
                $status=True;
                break;
            }
        }
    ...
    if ($status===false)
    {
        ...
        $query = "INSERT INTO flight_schedule (source,destination,adatetime,ddatetime,fno) VALUES ('$source','$destination','$adatetime','$ddatetime',$fno)";
        $run = mysqli_query($conn, $query);
        if ($run)
            echo "<script>alert('Flight Scheduled From : $source To : $destination');</script>";
        if ($coe>0)
        {
            ...
            $sql1="SELECT seq_no FROM flight_schedule WHERE fno='$fno' AND adatetime='$adatetimeFormatted' AND ddatetime='$ddatetimeFormatted' ORDER BY seq_no DESC LIMIT 1";
            $result1=mysqli_query($conn,$sql1);
            $row1 = mysqli_fetch_assoc($result1);
            $seq_id = $row1['seq_no'];

            $query = "INSERT INTO seat_cost (price,date,class_id,classname,seq_no) VALUES ($coe,NOW(),$class_id,'Economy Class',$seq_id)";
            $run = mysqli_query($conn, $query);
            // if ($run)
            //     echo "<script>alert('Flight Scheduled $seq_id $class_id');</script>";
        }
        else
        {
            $noofseatsofeco=0;
        ...
        if ($cof>0)
        {
            ...
            $query = "INSERT INTO seat_cost (price,date,class_id,classname,seq_no) VALUES ($cof,NOW(),$class_id,'First Class',$seq_id)";
            $run = mysqli_query($conn, $query);
            // if ($run)
            //     echo "<script>alert('Flight Scheduled $seq_id $class_id');</script>";
        }
        ...
        if ($cob>0)
        {
          ...
          $query = "INSERT INTO seat_cost (price,date,class_id,classname,seq_no) VALUES ($cob,NOW(),$class_id,'Business Class',$seq_id)";
          $run = mysqli_query($conn, $query);
          // if ($run)
          //     echo "<script>alert('Flight Scheduled $seq_id $class_id');</script>";
        }
        ...
        $query = "INSERT INTO flight_availabality (seatsofeco,seatsofbsn,seatsoffrt,seq_no) VALUES ($noofseatsofeco,$noofseatsofbsn,$noofseatsoffir,$seq_id)";
        $run = mysqli_query($conn, $query);
        // if ($run)
...
<body class="g-sidenav-show  bg-gray-200">
  ...
  <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
    ...
    <form class="heading" action="schedule-flight.php" method='POST' onsubmit="return validateDateTime()">
      <div class="container">
        <br>
        <div class="row">
          <div class="col-md-12">
            <div class="card">
              <div class="card-header">
                ...
                <small class='form-text text-muted' style="font-size:15px;">(If Flight added already and you want to
                  update then click on <a href="scheduled-flight-details.php">update flight</a> )</small>
              </div>
              <div class="card-body">
                    ...
                    if (isset($_GET['ig']))
                    {
                      ...
                      <tbody>
                        <tr>
                          ...
                          <td><b>:</b></td>
                          <td>
                            <select class="box" name="aname" id="air" readonly>
                        ...
                        <tr>
                          ...
                          <td><b>:</b></td>
                          <td>
                            <select class="box" id="flight" name="fno" readonly>
                              <option value="<?php echo $fno;?>" selected><?php echo $fno;?></option>
                            </select>
                          </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                          ...
                          <td><b>:</b></td>
                          <td><input type="number" class="box" name="noes" id="noes" min='20' max='40' value="<?php echo $noofseatsofeco;?>" readonly></td>
                        </tr>
                        <tr>
                            <td><b>Enter Number of Business Class Seats</b></td>
                            <td><b>:</b></td>
                            <td><input type="number" class="box" name="nobs" id="nobs" min='10' max='30' value="<?php echo $noofseatsofbsn;?>" readonly></td>
                        </tr>
                        <tr>
                            <td><b>Enter Number of First Class Seats</b></td>
                            <td><b>:</b></td>
                            <td><input type="number" class="box" name="nofcs" id="nofcs" min='15' max='35' value="<?php echo $noofseatsoffrt;?>" readonly></td>
                        </tr>
                        <tr>
                          ...
                          <td>
                            <select name="source" id="select1" onchange="getSelectValue(this.value)">
                              ...
                              <option value="BGL">Bengaluru (BGL)</option>
                              <option value="KOC">Kochi (KOC)</option>
                        ...
                        <tr>
                          ...
                          <td><b>:</b></td>
                          <td>
                            <input type="datetime-local" class="box" id="adatetime" name="adatetime" required>
                          </td>
                        </tr>
                        <tr>
                    ...
                    else
                    {
                        ...
                        <tbody>
                          <tr>
                            ...
                            <td><b>:</b></td>
                            <td>
                              <select class="box" name="aname" id="air">
                                <option id="" value="" selected disabled>Select Airline</option>
                              </select>
                              &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                              &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                              &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                          ...
                          <tr>
                            ...
                            <td><b>:</b></td>
                            <td>
                              <select class="box" id="flight" name="fno">
                                <option id="" value="" selected disabled>Select Flight</option>
                              </select>
                            </td>
                          </tr>
                          <tr>
                            ...
                            <td><b>:</b></td>
                            <td>
                              <input type="checkbox" id="eco" name="eco" checked disabled onchange="toggleSeatsVisibility()">
                              <label for="eco">Economy Class</label>&nbsp;&nbsp;&nbsp;&nbsp;
                              <input type="checkbox" id="bsn" name="bsn" checked disabled onchange="toggleSeatsVisibility()">
                              <label for="bsn">Business Class</label>&nbsp;&nbsp;&nbsp;&nbsp;
                              <input type="checkbox" id="fir" name="fir" checked disabled onchange="toggleSeatsVisibility()">
                              <label for="fir">First Class</label>&nbsp;&nbsp;&nbsp;&nbsp;
                            </td>
                          </tr>
                          <tr>
                            <td><b>Enter Number of Economy Class Seats</b></td>
                            <td><b>:</b></td>
                            <td><input type="number" class="box" name="noes" id="noes" min='20' max='40' disabled required>
                            </td>
                          </tr>
                          <tr>
                            <td><b>Enter Number of Business Class Seats</b></td>
                            <td><b>:</b></td>
                            <td><input type="number" class="box" name="nobs" id="nobs" min='10' max='30' disabled required>
                            </td>
                          </tr>
                          <tr>
                            <td><b>Enter Number of First Class Seats</b></td>
                            <td><b>:</b></td>
                            <td><input type="number" class="box" name="nofcs" id="nofcs" min='15' max='35' disabled required>
                            </td>
                          </tr>
                          <tr>
                            <td><b>Select Source </b></td>
